...
// Copyright (c) 2011-2012 ParanoiaCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("ParanoiaCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ParanoiaCoinTEST0\r");
            Send(hSocket, "WHO #ParanoiaCoinTEST0\r");
        } else {
            // randomly join #ParanoiaCoin00-#ParanoiaCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #ParanoiaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ParanoiaCoin%02d\r", channel_number).c_str());
        }
