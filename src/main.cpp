...
uint256 hashGenesisBlock("0x0d93adfb16e5d8cb7837b6f286e5479a074d3fce769814b279b3f86813bab1cd");
...
const string strMessageMagic = "ParanoiaCoin Signed Message:\n";
...
{
    // ParanoiaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 65;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 65;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 1.1 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 1.1 * 2.8;
...
{
    // ParanoiaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Paranoia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184713fa689ad5023690c80f3a49c8f23f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1438795147;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x8309a62aa9ead21369e606ceadb4a4b8cd6203f8c3533dced16feb9c57030315"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd3, 0xc3, 0x3f, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ParanoiaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ParanoiaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ParanoiaCoinMiner(CWallet *pwallet)
{
    printf("ParanoiaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ParanoiaCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ParanoiaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ParanoiaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ParanoiaCoinMiner, pwallet));
}
